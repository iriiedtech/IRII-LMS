...
export async function GET() {
  try {
    ...
    // Fetch all comments joining user, lesson, module, course
    ...
    const formattedComments = (comments || []).map((comment: any) => ({
      ...
      lesson_title: comment.lessons?.title || "Unknown Lesson",
      course_title: comment.lessons?.modules?.courses?.title || "Unknown Course",
      course_id: comment.lessons?.modules?.courses?.id || ""
    }));
